...
## Prototype Context (2026-08-12)

- Product: 晨曦大学 (fictional university) 宿舍管理系统 — mobile-only app for students, dorm staff, maintenance workers, counselors, and administrators.
- Visual direction: fresh blue-and-white campus style. Tokens: primary blue #2E7BD6, header gradient #4795DE → #F0F7FD, page background #F4F9FE, card white with 20px radius.
- Source visual target: `../concepts/chenxi_home_1.png` (ideation Option 1, student home screen).
- Scope decisions: no utilities/fees module; core journey prototyped is 首页 → 我要报修 → 提交成功 → 返回首页; 服务/消息/我的 tabs and 公告详情 are light placeholders per the confirmed module list.
- Roles (all five): 学生端 (home/report/services/messages/profile), 宿管端 (工作台/查寝/入住办理/工单/我的), 维修端 (工作台/工单/我的, with 接单/完工 flows), 辅导员端 (工作台/申请审批/查寝结果/我的, 学生住宿 accessed via workbench quick entry), 管理员端 (统计/楼栋/分配/账号/我的). Role switches live on each role's 我的 page. Personas: 林晓彤 (student), 王秀兰 (dorm manager), 李建国 (maintenance), 陈静 (counselor), 赵建军 (system admin).
- Navigation model: each role's bottom tabs are tab roots (keep the tab bar); flow pages (report/move-in/order detail/results) are pushed screens with headers and no tab bar. 查寝 is a tab root with an inline submit button (2026-08-12 fix).
- List pages carry enough mock data to scroll (messages 8, orders 9, students 14, approvals 8, check results 7, checkout 6); search/filter bars on list pages use the sticky-header pattern; the iOS bottom safe-area strip is masked globally.
- Role entry is by URL path: `/student`, `/manager`, `/maintenance`, `/counselor`, `/admin` (default `/` = student). 我的 pages are intentionally button-free (profile card + static info rows + version footer).
- Search bars on filterable pages are real inputs (student services, counselor students, admin accounts) using the shared `SearchBarInput` (KeyboardInput + clear button); every filterable list has an `EmptyState` fallback. Check-in result, move-in lookup, and dashboard stat/todo counts are derived from live component/data state, not hardcoded. Tappable rows/chips/buttons carry active (pressed) feedback states.
- QA record: `design-qa.md` in this project root. Verification method: Playwright capture + Qwen vision crop checks + DOM/geometry measurements.
- Implementation notes: Vite 8 requires Node 20+, so use the bundled runtime node (C:\Users\97412\AppData\Local\OpenAI\Codex\runtimes\cua_node\23828fd353da361d\bin\node.exe) for dev/build; dev server runs on port 5174 (5173 is occupied by another project).
